...
RenderInputMeta::RenderInputMeta(int _position, const std::string &_name,
                                 ...
                                 bool _divisor)
    : position(_position), name(_name), data(_data), nelements(_nelements),
      element_length(_element_length), element_type(_element_type),
      divisor(_divisor) {}
...
RenderPass::RenderPass(
  ...
  for (int i = 0; i < input.getNBuffers(); i++) {
    ...
    if (meta.isInteger()) {
      CHECK_GL_ERROR(glVertexAttribIPointer(meta.position, meta.element_length,
                                            meta.element_type, 0, 0));
    } else {
      CHECK_GL_ERROR(glVertexAttribPointer(meta.position, meta.element_length,
  ...
  for (size_t i = 0; i < uniforms.size(); i++) {
    CHECK_GL_ERROR(unilocs_[i] =
                       glGetUniformLocation(sp_, uniforms[i].name.c_str()));
  }
...
void RenderPass::initMaterialUniform() {
  ...
  for (size_t i = 0; i < input_.getNMaterials(); i++) {
    ...
    auto diffuse_data = [&ma]() -> const void * { return &ma.diffuse[0]; };
    auto ambient_data = [&ma]() -> const void * { return &ma.ambient[0]; };
    auto specular_data = [&ma]() -> const void * { return &ma.specular[0]; };
    auto shininess_data = [&ma]() -> const void * { return &ma.shininess; };
...
void RenderPass::createMaterialTexture() {
  ...
  for (size_t i = 0; i < input_.getNMaterials(); i++) {
    ...
    std::cerr << __func__ << " load data into texture " << tex << " dim: " << w
              << " x " << h << std::endl;
...
void RenderPass::updateVBO(int position, const void *data, size_t size) {
  ...
  if (bufferid < 0)
    throw __func__ + std::string(": error, can't find buffer with position ") +
